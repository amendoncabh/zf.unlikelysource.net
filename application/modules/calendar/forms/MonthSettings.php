<?php
class Calendar_Form_MonthSettings extends Zend_Form
{
	public function init()
	{
    	...
    	if (isset($session->locale)) {
    		list($langCode, $language, $countryCode, $country) = $this->_explode('_', $session->locale, $langList, $countryList);
			if ($langCode) {
				$localeChoices[$session->locale] = $session->locale . ':' . $language . ':' . substr($country, 0, 12);
			}
    	}
		...
		$weekdayFormat->setLabel('Weekday Format')
	  	...
	  	$months = array();
	  	for ($x = 1; $x <= 12; $x++) {
	  		$months[$x] = $locale->getTranslation($x, 'month', $langCode);
	  	}
	    ...
	    $submit = new Zend_Form_Element_Submit('Save');
	  	$submit->setLabel('Save')
	  		   ->setIgnore(TRUE);
	  	$this->addElements(array($changeLocale, $weekdayFormat, $monthFormat, $month, $year, $submit));
	}
